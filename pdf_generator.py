...
def generate_bdc_pdf(bdc, config, avec_reception=False):
    ...
    numero_date_data = [
        [Paragraph(f"<b>N° BON :</b> {num_str}", S['label']),
         Paragraph(f"<b>DATE CRÉATION :</b> {bdc.date_creation.strftime('%d/%m/%Y')}", S['label'])],
        [Paragraph(statut_imp_html, S['label']), '']
    ]
    ...
    # GESTION DU FILIGRANE (WATERMARK) POUR LES DUPLICATAS
    def on_page_callback(canvas, document):
        nb_imp = getattr(bdc, 'nb_impressions', 0) or 0
        if nb_imp > 1:
            canvas.saveState()
            ...
            canvas.rotate(45)
            canvas.drawCentredString(0, 20, f"DUPLICATA N° {nb_imp}")
            canvas.setFont('Helvetica-Bold', 18)
            canvas.setFillColor(colors.Color(0.85, 0.15, 0.15, alpha=0.12))
            canvas.drawCentredString(0, -15, f"(Imprimé {nb_imp} fois)")
            canvas.restoreState()
...
def generate_bdc_image(bdc, config, avec_reception=False, dpi=160, format='jpeg'):

    """

    Génère une image haute définition (JPEG ou PNG) du Bon de Commande officiel TAXI GAB+

    à partir du rendu vectoriel PDF via PyMuPDF.

    Retourne les octets (bytes) de l'image prête à l'envoi ou l'affichage.
    ...
    # Si le document fait plusieurs pages, on assemble verticalement les pages
