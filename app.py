...
def create_app():
    ...
    def bdc_envoyer_whatsapp_serveur(id):
        ...
        if succes:
            ...
            bdc.date_envoi_whatsapp = datetime.now()
            enregistrer_action(bdc, 'envoi_whatsapp',
                               details=f"Image officielle transmise à {bdc.fournisseur_whatsapp} ({detail}) par {nom_user}")
            db.session.commit()
            ...
            flash(f"⚠️ Erreur de transmission d'image : {detail}.", "warning")
